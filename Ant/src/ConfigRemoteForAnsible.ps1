...
netsh advfirewall firewall set rule name="Windows Remote Management (HTTP-In)" new profile='public,domain' remoteip=any enable=no

C:\Windows\System32\cmd /c 'winrm set winrm/config/service @{AllowUnencrypted="true"}'

winrm qc -q

start WinRM

write "Enabled firewall rules and restarted WinRM service"
